 ...
 */
void QgsServer::init(int & argc, char ** argv)
{
  ...
  mServerContext.initQgsApplication( argc, argv, getenv( "DISPLAY" ) );
  ...
  mServerContext.mapRenderer()->setLabelingEngine( new QgsPalLabeling() );
  ...
  // Init plugins
  if ( ! QgsServerPlugins::initPlugins( &mServerContext.serverInterface( ) ) )
  {
  ...
  mServerContext.initPluginFilters( );
#endif
...
QByteArray QgsServer::handleRequest(const QString queryString ,
  ...
  mServerContext.qgsApplication()->processEvents();
  if ( logLevel < 1 )
  {
    time.start();
    printRequestInfos();
  ...
  mServerContext.serverInterface().setRequestHandler( theRequestHandler.data() );
  // Iterate filters and call their requestReady() method
  QgsServerFiltersMap::const_iterator filtersIterator;
  for ( filtersIterator = mServerContext.pluginFilters().constBegin(); filtersIterator != mServerContext.pluginFilters().constEnd(); ++filtersIterator )
  {
  ...
  theRequestHandler->setPluginFilters( mServerContext.pluginFilters( ) );
#endif
  ...
  //Config file path
  QString configFilePath = configPath( mServerContext.configFilePath(), parameterMap );

  mServerContext.setConfigFilePath( configFilePath );
  ...
  if ( !theRequestHandler->exceptionRaised() )
  {
    ...
    {
      QgsWCSProjectParser* p = QgsConfigCache::instance()->wcsConfiguration( mServerContext.configFilePath( ) );
      if ( !p )
      {
      ...
      {
        QgsWCSServer wcsServer( mServerContext.configFilePath( ), parameterMap, p, theRequestHandler.data() );
        wcsServer.executeRequest();
      }
    }
    else if ( serviceString == "WFS" )
    {
      QgsWFSProjectParser* p = QgsConfigCache::instance()->wfsConfiguration( mServerContext.configFilePath( ) );
      if ( !p )
      {
        theRequestHandler->setServiceException( QgsMapServiceException( "Project file error", "Error reading the project file" ) );
      }
      ...
      {
        QgsWFSServer wfsServer(  mServerContext.configFilePath( ), parameterMap, p, theRequestHandler.data() );
        wfsServer.executeRequest();
      }
    }
    else if ( serviceString == "WMS" )
    {
      QgsWMSConfigParser* p = QgsConfigCache::instance()->wmsConfiguration(  mServerContext.configFilePath( ), parameterMap );
      if ( !p )
      {
      ...
      {
        QgsWMSServer wmsServer(  mServerContext.configFilePath( ), parameterMap, p, theRequestHandler.data(), mServerContext.mapRenderer(), & mServerContext.capabilitiesCache( ) );
