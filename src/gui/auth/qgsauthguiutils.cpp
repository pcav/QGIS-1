...
void QgsAuthGuiUtils::passwordHelperSync( QgsMessageBar *msgbar, int timeout )
{
  ...
  if ( ! QgsAuthManager::instance()->masterPasswordIsSet( ) )
  {
    msg = QObject::tr( "Master password is not set and cannot be store in your %1" )
          .arg( QgsAuthManager::AUTH_PASSWORD_HELPER_DISPLAY_NAME );
    level = QgsMessageBar::WARNING;
  }
...
void QgsAuthGuiUtils::passwordHelperEnable( bool enable, QgsMessageBar *msgbar, int timeout )
{
  QgsAuthManager::instance()->passwordHelperEnable( enable );
  QString msg = enable ? QObject::tr( "Your %1 will be <b>used from now</b> on to store and retrieve the master password." )
                .arg( QgsAuthManager::AUTH_PASSWORD_HELPER_DISPLAY_NAME ) :
